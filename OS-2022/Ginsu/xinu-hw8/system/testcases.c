...
void testcases(void)
{
    ...
    kprintf("3) Test sc_create syscall\r\n");
	...
	PrintFreeList();
	ulong *first = sc_getmem(1000);
	kprintf("Number first = 0x%x\r\n", first);
	kprintf("--------------------After 1st sc_getmem(1000)--------------------\r\n");
	PrintFreeList();
	ulong *second = sc_getmem(1000);
	kprintf("--------------------After 2nd sc_getmem(1000)--------------------\r\n");
	PrintFreeList();
	sc_getmem(500);
	kprintf("--------------------After 3rd sc_getmem(500)--------------------\r\n");
	PrintFreeList();
	kprintf("----------------sc_freemem(0x%08x, 1000) is called:----------------\r\n", first);
	sc_freemem(first, 1000);
	PrintFreeList();
	kprintf("----------------freemem(0x%08x, 500) is called:----------------\r\n", second);
	sc_freemem(second, 500);
	PrintFreeList();
	break;
    case '8' :
	kprintf("The original free List:\r\n");
