...
static int sysinit(void)
{
    ...
    // TODO: Set the tickets field on the null process --> means: the null process is the first process. Thus, we have to set tickets to default to 1 so that we have a process to start with. The ticket should not be less than 1 because you want to give an oppurtunity to the process to even run in the first place thus a process can not get 0 tickets.
    ppcb->tickets = 1;
