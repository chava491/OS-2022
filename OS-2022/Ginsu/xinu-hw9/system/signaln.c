...
syscall signaln(semaphore sem, int count)
{
    ...
    if(semptr->count < 0){ 					// Here we check if the semaphore count is negative.
								// If so, that means |count| threads are waiting on the semaphore.	
       for (c = semptr->count; c < 0; c++) { 			// We wake the processes until the count is no longer 0.
    ...
    semptr->count+=count;					// We increment the semaphore 'count' times.
    resched();
    restore(im);
    return OK;
}
